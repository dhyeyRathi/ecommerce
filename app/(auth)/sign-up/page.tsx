...
export default function SignUpPage() {
  ...
  const handleMatch = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
      ...
      >
        ...
        <form aria-label="input container" className="flex flex-col w-full gap-6 lg:gap-10 lg:p-6 justify-center items-center">
          <div className="flex flex-col md:flex-row w-full md:gap-4 md:w-2/3 md:items-center  text-heading">
                <label className="md:text-center md:w-30" htmlFor="name input" >
                    Username:
                </label>
            ...
            <div className="flex flex-col md:flex-row md:gap-4 w-full md:w-2/3  md:items-center relative  text-heading">
                <label className="md:text-center md:w-30" htmlFor="re-password input" >
                   Re type Password:
                </label>
                <input type="password" value={rePassword} id="re-password input" onChange={(e) => setRePassword(e.target.value)}
                className="bg-background border-2 border-border focus:shadow-xs shadow-text inset-shadow-[0_0_4px] w-75 md:w-120  inset-shadow-border 
               p-2  rounded-lg focus:outline-none" 
                placeholder="Re-enter your password..." required/>
                {matchPwd && <p className="absolute text-red-600 text-sm md:text-base bottom-[-25] lg:bottom-[-30] right-32">Passwords do not match!</p>}
            </div>
